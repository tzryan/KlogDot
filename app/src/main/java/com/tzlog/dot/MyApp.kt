...
class MyApp :Application(){
    ...
    override fun onCreate() {
        ...
        KLog.init(this)
            ...
            .setFilePrefix(".KLog")// 当文件前缀为空时，默认为"KLog"，android以点开头，则是隐藏文件，即写入文件为"klog-MM-dd.txt"
            .setFileSuffix(".log")// 当文件后缀为空时，默认为".txt"，即写入文件为"klog-MM-dd.txt"
            .setSingleTagSwitch(false)// 一条日志仅输出一条，默认开，为美化 AS 3.1.0 的 Logcat
            .setSaveDays(2f)//0.0007f 设置日志可保留天数，默认为 -1 表示无限时长  (1分钟=0.00069f)
